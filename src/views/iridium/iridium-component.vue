...
// import CustomHeader from "@/views/iridium/custom-header.vue";
import LeftTemplate from "@/views/iridium/left-template.vue";
import MainTemplate from "@/views/iridium/main-template.vue";
import RightTemplate from "@/views/iridium/right-template.vue";
...
async function handleStatusChange(Status: string, modelSize: number) {
  try {
    ...
    const url = `ws://${process.env.NODE_ENV === "production" ? window.location.host : ''}${process.env.VUE_APP_WS_API}ws/iridium_group/`;
    if (Status === "false") {
